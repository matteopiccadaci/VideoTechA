...
<input class="form-control form-control-sidebar" type="search" id="cercaanno" onkeyup="myFunction4()" placeholder="Cerca per data..." title="Cerca Anno">
        ...
        <?php
        $queryacquisti="SELECT Acquisti_album.data_acquisto, Album.nome_album, Acquisti_album.quantita, Musicisti.nome_musicista, Album.prezzo_acquisto, Clienti.nome, Clienti.cognome, Acquisti_album.cliente
        FROM Acquisti_album JOIN Album on Acquisti_album.articolo=Album.id_album JOIN Musicisti on Album.musicista=Musicisti.id_musicista JOIN Clienti on Acquisti_album.cliente=Clienti.id_cliente
            ...
            if($ute=$connex_db->query($queryacquisti)){
                while($arrute=mysqli_fetch_array($ute, MYSQLI_ASSOC)) {
                    echo "<tr><th scope='row'>    " .$arrute['cliente']. " " . '-' . "  "   .$arrute['nome']. "    " .$arrute['cognome']. "    </th>
                            <th scope='row'>" .$arrute['nome_musicista']. "</th>
