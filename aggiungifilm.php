...
session_start();
$id=$_SESSION['id'];
$queryartisti="SELECT nome, cognome
FROM Registi";
...
if(isset($_POST['conferma'])){
    ...
    $key=$_POST['key'];
    if ($key!='I%$hmn56hy!'){
        echo "<script> alert ('Inserimento non avvenuto: Qualcosa è andato storto...');
    window.location.replace('/aggiungiregista.php')</script>";
    }
    else
    {$querycodice="SELECT id_regista
    FROM Registi 
        WHERE cognome='$regista'";
        $pos=$connex_db->query($querycodice);
        $arrid=mysqli_fetch_array($pos, MYSQLI_ASSOC);
        $idregista=$arrid['id_regista'];
        $queryaggiunta= "INSERT INTO Film  (nome_film, regista, genere, anno_pubblicazione, quantita_copie, prezzo_acquisto, produttore)
                VALUES ('$nomefilm', '$idregista','$genere','$anno','$quantita','$prezzo','$casa')";
        if ($result=$connex_db->query($queryaggiunta)){
            echo "<script> alert('Film inserito correttamente!'); window.location.replace('/catalogofilmadmin.php')</script>";
        }
    ...
    <div class="card">
        <div class="card-body login-card-body">
            ...
            <form method="post">
                <div class="row g-3">
                    <div class="col-md-12">
                        <label for="film" class="form-label">Nome Film</label>
                        <input type="text" class="form-control" id="film" placeholder="Inserisci il nome del film" name="film">
                    </div>
                    <div class="col-md-12">
                        ...
                        <select class="form-select" aria-label="Default select example" id="regista" name="regista">
                            ...
                            {
                                echo "<option>" . $artisti['cognome'] . "</option>";
                            }
                    ...
                    <div class="col-md-12">
                        <label for="casa" class="form-label">Produttore</label>
